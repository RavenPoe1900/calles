...
async def _calles_cercanas(
    ...
    """Todas las calles con nombre dentro del radio, vía Overpass (una sola llamada)."""
    query = f"""
    [out:json][timeout:25];
    way(around:{radio_m},{lat},{lon})["highway"]["name"];
    out tags;
    """
    r = await client.post(OVERPASS_URL, data=query, headers=HEADERS, timeout=30)
    r.raise_for_status()
    data = r.json()
    nombres = {
        el["tags"]["name"]
...
async def entrecalles(
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            raise HTTPException(status_code=502, detail=f"Error consultando Overpass: {e}")
